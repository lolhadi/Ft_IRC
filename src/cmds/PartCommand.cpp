...
// Leaving the Channel but not the server
void PartCommand::execute(Client *client, std::vector<std::string> arguments) {
	...
	// Assemble reason, if got reason input
	if (arguments.size() >= 2)
	...
	// this part look for the user in the channle
	Channel *chan;
	while (it != chans.end())
	{
		chan = it.operator*();
	...
	// if have itarate until the end means not in channel
	if (it == chans.end())
	{
