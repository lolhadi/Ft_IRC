...
// e.g: KICK #test BADguy :You are Rude
void KickCommand::execute(Client *client, std::vector<std::string> arguments)
{
	...
	// take the channel name[0], the user [1] and the reason
	std::string chan_name = arguments[0];
	std::string target = arguments[1];
	std::string reason = "No reason specified.";

	// Assemble reason
	if (arguments.size() >= 3) // if there anything after channel + user, treat as reason
	{
		reason = "";// if got reason replace
		for (std::vector<std::string>::iterator it = arguments.begin() + 2; it != arguments.end(); it++)
			reason.append(*it + " ");
		// why this because the reason is consider is argument, so this make it in one string
	}
	// remove first :
	if (reason[0] == ':')
