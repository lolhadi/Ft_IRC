...
Channel::Channel(std::string const &name, std::string const &password, Client *admin, Server *server)
					: _name(name) , _admin(admin), _l(1000), _i(false), _k(password),_topic(""),_t(true), _server(server) {}
Channel::~Channel() {}
...
// 1. Check if a client is inside this channel
bool Channel::hasClient(Client *client)
{
	// Loop through your list of clients (assuming it's called _clients or similar)
...
// 4. Check if topic is protected (+t mode)
bool Channel::isTopicProtected() const
{
	return (this->_t);
}

// 5. Set topic protection (used by MODE command later)
void Channel::setTopicProtection(bool mode)
{
	this->_t = mode;
}
